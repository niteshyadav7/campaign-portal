          ...
          <div className="grid gap-5 lg:grid-cols-[1fr_320px]">
            <div className="space-y-5">
              <div className="rounded-lg border border-dashed border-slate-300 bg-slate-50 p-4">
                <Label htmlFor="campaign-csv-file" className="text-sm font-black text-slate-800">CSV file</Label>
                <input
              ...
              <div className="space-y-2">
                <Label htmlFor="campaign-csv-paste" className="text-sm font-black text-slate-800">Or paste CSV</Label>
                <textarea
            ...
            <aside className="rounded-lg border border-slate-200 bg-slate-950 p-4 text-white">
              ...
              <p className="text-sm font-medium text-slate-400">campaign rows detected</p>
              <div className="mt-5 rounded-lg border border-white/10 bg-white/[0.07] p-3">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {extraHeaders.length > 0 ? extraHeaders.map((header) => (
                    <span key={header} className="rounded-full bg-blue-300/10 px-2.5 py-1 text-xs font-bold text-blue-200">
                      {header}
                    </span>
                  )) : (
              ...
              {previewRows[0] ? (
                <div className="mt-4 rounded-lg border border-white/10 bg-white/[0.07] p-3 text-sm">
                  <p className="font-black">{previewRows[0].name || 'Unnamed campaign'}</p>
                  <p className="mt-1 text-slate-400">{previewRows[0].brand || previewRows[0].brand_id || 'No brand mapped'}</p>
                </div>
              ) : null}
              {error ? (
